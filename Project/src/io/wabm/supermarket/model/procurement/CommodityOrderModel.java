...
public class CommodityOrderModel<T> extends TableViewModel<T> {
    ...
    private String kPassSQLWithID = "UPDATE `order`  SET status=2 WHERE `order`.order_id = ?; ";

    private String kDeleteSQLWithID ="DELETE `order` FROM `order` WHERE `order`.order_id = ?; ";
    ...
    public void CreateOrder(String SupplierID, String commodityID, int quantity, Double price, Callback<Boolean, Void> callback) {
        ...
        try {
            int order_num = jdbcOperations.queryForObject("SELECT count(*) order_num FROM `order` WHERE status = 1 and supplier_id = ?;", Integer.class,SupplierID);

            if (order_num != 0) {
                ConsoleLog.print("update procurement record");
                int orderID = jdbcOperations.queryForObject("SELECT order_id orderID FROM `order` WHERE status = 1 and supplier_id = ?;", Integer.class,SupplierID);
                jdbcOperations.update("INSERT INTO order_detail (order_id,commodity_id,quantity,price_db) VALUE (?,?,?,?);",orderID,commodityID,quantity,price);
                jdbcOperations.update("DELETE  from procurement_requirement where commodity_id = ?", commodityID);
            } else {
                ConsoleLog.print("insert order_detail");
                // insert inventory detail
                jdbcOperations.update("INSERT INTO `order` (employee_id,supplier_id,status) VALUE (4,?,1);", SupplierID);
